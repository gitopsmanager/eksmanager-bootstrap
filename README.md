...
**`apply` fails with `EntityAlreadyExists` on `aws_iam_openid_connect_provider.github_actions`** — the shared services account already has a GitHub Actions OIDC provider (an account can only hold one per URL), and the script's pre-flight check did not see it. That check is best-effort: it assumes `SHARED_SERVICES_ROLE_NAME` and lists providers, so it is skipped when the AWS CLI is missing and fails quietly if the assume-role does. Nothing is written to state on a failed create, so there is nothing to clean up. Find the existing one:
